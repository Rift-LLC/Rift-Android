...
if getgenv().IS_UI_LOADED then
	maketoast("The exploit is already running!")
	return
end
getgenv().IS_UI_LOADED = true

local g = cloneref(game)
local getService = clonefunction(g.GetService)
local rift = {
	protectedService = function(service)
		return cloneref(getService(g, service))
	end,
	
	autoexecute = clonefunction(client.enableautoexec),
	runcode = clonefunction(client.execute),
	securestring = clonefunction(client.securestring)
}

getgenv().client = nil;

local UserInputService = rift.protectedService("UserInputService")
